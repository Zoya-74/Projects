...
class Customer:
    ...
    def make_call(self, call: Call) -> None:
        ...
        """
        # Use the source number of this call since we are making a call
        number = call.src_number

        for line in self._phone_lines:
            # Go through the phone lines of the customer and once we find a
            # match, make call with that phone line
    ...
    def receive_call(self, call: Call) -> None:
        ...
        """
        # Use the dst number since this customer is receiving the call, so
        # their number must be the dst number
        number = call.dst_number

        for line in self._phone_lines:
            # Go through the phone lines of the customer and once we find a
            # match, receive call with that phone line
            if line.get_number() == number:
