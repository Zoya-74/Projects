...
class PhoneLine:
    ...
    def make_call(self, call: Call) -> None:
        ...
        """
        # Add this call to the history of outgoing calls dictionary
        self.callhistory.register_outgoing_call(call)
        date = call.get_bill_date()

        # If there are no bills created for the month the call was made in
        # that means we need to advance to a new month
        if date not in self.bills:
            self.new_month(date[0], date[1])

        # Bill this call to the appropriate contract of this phone line
    ...
    def receive_call(self, call: Call) -> None:
        ...
        <call>.
        """
        # Add this call to the dictionary with all the incoming calls
        self.callhistory.register_incoming_call(call)
        date = call.get_bill_date()

        # If the date is not in the bills dictionary, then a new month
        # has started, so advance to new month
        if date not in self.bills:
            self.new_month(date[0], date[1])
