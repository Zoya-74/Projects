...
class CallHistory:
    ...
    def register_outgoing_call(self, call: Call) -> None:
        ...
        # If there were other calls on the same date, add this call to the
        # dictionary key. (In outgoing calls)
        if date in self.outgoing_calls:
            self.outgoing_calls[date].append(call)

        # If not, create a new key with the date and add value of call
        else:
    ...
    def register_incoming_call(self, call: Call) -> None:
        ...
        # If date in the customers incoming dictionary, add this call to that
        # key
        if date in self.incoming_calls:
            self.incoming_calls[date].append(call)

        # If not, create a new key in the incoming calls dictionary and add call
        else:
