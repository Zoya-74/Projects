...
def process_event_history(log: dict[str, list[dict]],
    ...
    # Advance every customer to a new/start month, this is the first month
    # that shows up in the data set
    ...
    for event_data in log['events']:
        ...
        # Find and store the customers we need so we can make/receive call later
        cust1 = find_customer_by_number(event_data["src_number"],
                                        customer_list)
        cust2 = find_customer_by_number(event_data["dst_number"],
                                        customer_list)
        # if the event is a call, store relevant information to make that call
        if event_data['type'] == 'call':
            ...
            new_call = Call(event_data["src_number"], event_data["dst_number"],
                            billing_date, event_data["duration"], src_location,
                            dst_location)
            cust1.make_call(new_call)
            cust2.receive_call(new_call)

        # If a new month is encountered, advance all customers to the new month
