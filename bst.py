...
class BinarySearchTree:
    ...
    def items_in_range(self, start: Any, end: Any) -> list:
        ...
        if self.is_empty():
            ...
            items = []
            if start == self._root or end == self._root:
                items = [self._root]
            if start <= self._root <= end:
                item = (self._left.items_in_range(start, self._root) +
                         [self._root] + self._right.items_in_range(self._root, end))
                return item
            elif start >= self._root:
                items.extend(self._right.items_in_range(start, end))
                return items
            elif end <= self._root:
                item = self._left.items_in_range(start, end) + items
                return item
