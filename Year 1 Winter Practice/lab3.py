...
class Player():
    """Represents every player in number game
    === Attributes ===
...
class RandomPlayer(Player):
    """
    Pick a random move among the legal possibilities
    """
...
class UserPlayer(Player):
    """
    The user decides the amount of moves
    """

    def move(self, current: int, min_step: int, max_step: int,
             goal: int) -> int:
        amount = int(input(f"Enter a number between {min_step} and {max_step}"))
        while amount < min_step or amount > max_step:
            amount = int(
                input(f"Enter a number between {min_step} and {max_step}"))
        return amount


class StrategicPlayer(Player):
    """
    This class makes moves strategically.
    """

    def move(self, current: int, min_step: int, max_step: int,
             goal: int) -> int:
        # try to get to the number goal - 2(max_step) - 1
        num = goal - max_step - 1
        num2 = goal - 2 * max_step - 1
        if current < num2:
            return min_step
        elif num2 <= current <= num:
            return goal - current
...
def make_player() -> Player:
    ...
    """
    generic_name = input(f'Enter the type of player you want:')
    while generic_name not in ("StrategicPlayer", "UserPlayer", "RandomPlayer"):
        generic_name = input(
            f'Please enter a new name that is either "Strategic'
            f'Player", "UserPlayer", or "RandomPlayer"')
    name = input(f'Enter a name for {generic_name}: ')
    if generic_name == 'StrategicPlayer':
        return StrategicPlayer(name)
    elif generic_name == 'UserPlayer':
