...
class BoolOp(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        """
        if self.op == 'and':
            return all([vals.evaluate() for vals in self.values])
        elif self.op == 'or':
            return any([vals.evaluate() for vals in self.values])
...
class Compare(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        prev = self.left.evaluate()
        # Iterate over comparisons
        for op, expr in self.comparisons:
            # Evaluate the expression
            value = expr.evaluate()
            # Check the operator and perform comparison
            if op == '<':
                result = result and (prev < value)
            elif op == '<=':
                result = result and (prev <= value)
            # Update left operand for the next comparison
            prev = expr
        return result
